...
def expanded_L1_norm(galaxy0, galaxy1, emptyrows, emptycols, factor=2):
    factor = factor - 1
    x0, y0 = galaxy0
    x1, y1 = galaxy1
    if x1 < x0:
    ...
    return (x1 - x0 + nx*factor) + (y1 - y0 + ny*factor)
...
for i, galaxy1 in enumerate(galaxies):
    for j, galaxy2 in enumerate(galaxies[i+1:]):
        distance = expanded_L1_norm(galaxy1, galaxy2, 
                                    emptyrows, emptycols, factor=1000000)
        s += distance
print(s)
