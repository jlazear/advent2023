...
fn expanded_L1_norm(galaxy0: &Coord, galaxy1: &Coord, emptyrows: &[usize], emptycols: &[usize], factor: i64) -> i64 {
    let factor = factor-1;
    ...
    let norm = (x1 as i64 - x0 as i64 + nx*factor) + (y1 as i64 - y0 as i64 + ny*factor);
    return norm;
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (i, galaxy1) in galaxies.iter().enumerate() {
        for galaxy2 in &galaxies[i+1..] {
            let distance = expanded_L1_norm(galaxy1, galaxy2, &emptyrows, &emptycols, 1000000);
            s += distance;
        }
    }
